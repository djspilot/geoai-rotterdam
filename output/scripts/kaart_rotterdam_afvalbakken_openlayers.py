"""Interactieve OpenLayers-kaart van afvalbakken in heel Rotterdam.

Genereert een zelfstandig HTML-bestand (OpenLayers via CDN) met OSM-basiskaart,
alle TIR-gebiedsgrenzen en geclusterde afvalbak-punten met klik-popups.
...
from rotterdam import load_layer, MAPS_DIR, WGS84
...
# Alle afvalbakken van Rotterdam; alle gebieden als grens.
pts = afval[[c for c in POPUP_FIELDS if c in afval.columns] + ["geometry"]].to_crs(WGS84)
grens = gebieden[["GEBDNAAM", "geometry"]].to_crs(WGS84)
...
<title>Afvalbakken in Rotterdam — interactief</title>
...
<div id="map"></div>
<div class="panel">
  <h1>Afvalbakken in Rotterdam</h1>
...
out = MAPS_DIR / "kaart_rotterdam_afvalbakken_openlayers.html"
out.write_text(html, encoding="utf-8")
print(f"Afvalbakken in Rotterdam: {len(pts)}")
print(f"SAVED: {out}")
