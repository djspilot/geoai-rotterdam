...
4. **Kaart heeft altijd** titel, legenda, projectie **en bronvermelding**. De bronvermelding staat in de footer via `finalize_map(source=...)` (samen met datum, AI-disclaimer en uitgever — zie invariant 11); geef er altijd de juiste bron mee, laat 'm nooit weg. Voor een basemap van derden komt daar de "Basiskaart: …"-vermelding bij (invariant 12). Noordpijl en schaalstok zijn optioneel (invarianten 13/14). `style_map(ax, title)` + `finalize_map(fig, source=...)` regelen dit.
...
16. **Getalnotatie volgens de regionale (NL) instellingen.** Gebruik in alle labels/legenda's/teksten de Nederlandse getalnotatie: **duizendtalscheiding met punt** en **decimaal met komma** (bv. `20.028`, `1.957`, `1,5 km`, `6.024 inw./km²`). Nooit de Engelse notatie (`20,028` / `1.5`). Formatteer met de gedeelde helper **`nl_getal(v, decimalen=0)`** uit `rotterdam` (`from rotterdam import nl_getal`) i.p.v. rauwe `str(getal)` of ad-hoc `f"{v:,}"`. `add_proportional_legend`/`add_swatch_legend` gebruiken deze notatie al standaard.
    ...
    **Bereik-labels in een klasse-legenda** (choropleet): een klasse wordt getoond als `ondergrens - bovengrens` — de twee grenzen gescheiden door een **streepje** (`" - "`), niet door een komma (`"0 - 6"`, niet `"0, 6"`), want een komma leest als opsomming/decimaal. Zijn de klassegrenzen op hele getallen afgerond (de default `integer_breaks=True`), toon de labels dan als **hele getallen** zonder decimalen. `choropleth()` regelt beide automatisch: bij `integer_breaks` worden de grenzen afgerond en als hele getallen geformatteerd, en de komma-scheiding wordt vervangen door een streepje. Bouw je zelf een klasse-legenda, hanteer dan dezelfde streepje-scheiding.
...
19. **Titel samenstellen.** De titeltekst volgt een vaste opbouw. Deze regels gaan over de *inhoud* van de titel; vorm/hiërarchie staat in invariant 17, plaatsing in invariant 18.
    1. **Werkverdeling hoofdtitel vs. subtitel.** De **hoofdtitel** benoemt het *wat* + het *waar*: het onderwerp en de locatie (bv. *"Afvalbakdichtheid per subbuurt — Rotterdam"*). De **subtitel** draagt de nuance die de kaart interpreteerbaar maakt: de **eenheid/normalisatie** en eventueel de **bron of periode** (bv. *"afvalbakken per km² (Obsurv)"*). Zet normalisatie-eenheden (per km², per 1.000 inw.) altijd in de subtitel, niet in de hoofdtitel.
    2. **Locatie expliciet.** Noem altijd Rotterdam, plus het gebied/de buurt als de kaart daarop is ingezoomd (bv. *"… — Rotterdam Centrum"*). Nooit alleen het thema zonder plaats.
    3. **Mensentaal, geen datavelden.** Gebruik begrijpelijke termen, nooit ruwe kolomnamen of codes uit de bron (`DOELGROEP`, `WIJK`, `afval_per_km2`). Vertaal naar wat het inhoudelijk betekent.
